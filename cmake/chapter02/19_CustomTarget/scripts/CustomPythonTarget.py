...
def main() -> None:
    ...
    parser.add_argument("--directory", help="Directory to search for source files", type=str, required=True)
    parser.add_argument("--extension", help="File extension to search for (e.g., .cpp)", nargs="?", type=str, default=".cpp")
    args = parser.parse_args()

    total_lines = 0
    for fname in glob.glob(f"{args.directory}/**/*{args.extension}", recursive=True):
        file_name = os.path.basename(fname)
        file_lines = num_lines(fname)
        print(f"Total lines of code: {file_name} = {file_lines}")
        total_lines += file_lines
    print(f"Total lines of code in directory {args.directory}: {total_lines}")
